...
public class BaseClass1 {
	...
	/*public static void getData(int r,int c) throws IOException {
		...
		int colcount = rs.getPhysicalNumberOfCells();
		
		
		for (int i = 0; i <rowcount ; i++) {
			Row row = s.getRow(i);
			for (int j = 0; j < colcount; j++) {
				Cell cell = rs.getCell(j);
				...
				if (type==0) {
					if (DateUtil.isCellDateFormatted(cell)) {
						...
						double bo = cell.getNumericCellValue();
						long l = (long) bo;
	...
	public static List<HashMap<String, String>> readData() throws IOException {
		...
		for (int i = 0; i < s.getPhysicalNumberOfRows(); i++) {
			...
			for (int j = 0; j < row.getPhysicalNumberOfCells(); j++) {
				...
				if (type == 1) {
					String name = cell.getStringCellValue();
					mp.put(headerrow.getCell(j),name);
